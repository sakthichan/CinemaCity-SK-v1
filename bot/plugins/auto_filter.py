...
async def auto_filter(bot, update):
    ...
    if filters:
        for filter in filters: # iterating through each files
            ...
            button_text = f"ℹ️{file_size} - 📂{file_name}"
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("𝐍𝐞𝐱𝐭 ⏩", callback_data=f"navigate(0|next|{query})")
                ]
            )
        
        # Just A Decaration
        result[0].append([
            InlineKeyboardButton(f"📀 𝐏𝐚𝐠𝐞 𝟏 /{len_result if len_result < max_pages else max_pages} 📀", callback_data="ignore")
        ])
        ...
        try:
            await bot.send_message(
                ...
                text=f"⚡ 𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲 @CC_links_SK.. \n\n 𝐅𝐨𝐮𝐧𝐝 {(len_results)} 𝐑𝐞𝐬𝐮𝐥𝐭𝐬 𝐅𝐨𝐫 𝐘𝐨𝐮𝐫 𝐑𝐞𝐪𝐮𝐞𝐬𝐭: <code>{query}</code>",
